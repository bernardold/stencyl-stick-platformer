...
if(wrapper.enabled && sameAsAny(getActorType(7), event.otherActor.getType(),event.otherActor.getGroup()))
{
        ...
        Engine.engine.setGameAttribute("havePencil", true);
        Engine.engine.setGameAttribute("tutorialPencil", true);
        runLater(1000 * 4, function(timeTask:TimedTask):Void {
                    Engine.engine.setGameAttribute("tutorialPencil", false);
}, actor);
}
});
...
if(wrapper.enabled && sameAsAny(getActorType(9), event.otherActor.getType(),event.otherActor.getGroup()))
{
        playSound(getSound(34));
        recycleActor(event.otherActor);
        Engine.engine.setGameAttribute("haveEraser", true);
        Engine.engine.setGameAttribute("tutorialEraser", true);
        runLater(1000 * 4, function(timeTask:TimedTask):Void {
                    Engine.engine.setGameAttribute("tutorialEraser", false);
}, actor);
}
});
